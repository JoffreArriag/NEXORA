export default function InicioCliente() {
  return (
    <div>
      <h1 className="text-5xl font-black text-slate-800">Inicio</h1>
      <p className="mt-4 text-lg text-slate-700">
        Explora nuestras categorías y productos destacados.
      </p>
      ...
      <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {["Electrónica","Ropa","Hogar","Juguetes","Deportes","Belleza"].map((cat, i)=>(
          <div key={i} className="bg-cyan-100 text-cyan-700 rounded-3xl p-8 flex items-center justify-center font-bold text-xl cursor-pointer hover:bg-cyan-200 transition-colors">
            {cat}
          </div>
        ))}
      </div>
      ...
      <h2 className="mt-16 text-3xl font-bold text-slate-800">Productos destacados</h2>
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {[...Array(6)].map((_, i)=>(
          <div key={i} className="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow cursor-pointer">
            ...
            <div className="p-6">
              <h3 className="text-2xl font-bold text-slate-800 mb-2">Producto {i+1}</h3>
