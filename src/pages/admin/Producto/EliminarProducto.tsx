...
export default function EliminarProducto() {
  ...
  const eliminarProducto = async () => {
    ...
    try {
      ...
      setToastMessage(`Producto "${productoSeleccionado.nombre}" eliminado`);
      setToastType("success");
      setProductoSeleccionado(null);
    } catch (error) {
      console.error(error);
      setToastMessage("Ocurrió un error al eliminar el producto");
      setToastType("error");
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-transparent">
      <h2 className="text-3xl font-bold mb-6 text-neutral-950">Eliminar Producto</h2>
      ...
      <div className="bg-white/10 p-8 rounded-3xl min-h-[400px] flex flex-col gap-4 overflow-y-auto">
        {!productoSeleccionado && sugerencias.length > 0 ? (
          ...
          // Formulario estilo EditarProducto (solo lectura)
